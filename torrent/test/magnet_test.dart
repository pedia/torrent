...
void main() {
  ...
  test('parse_invalid_escaped_hash_parameter', () {
    expect(() => AddParam.parse('magnet:?xt=urn%%3A'),
  ...
  test('parse_base32_hash', () {
    ...
    expect(d == p.infoHashes.v1, isTrue);
  }, skip: true); // TODO: 2.0
  ...
  test('parse_short_hash', () {
    expect(
      () => AddParam.parse('magnet:?xt=urn:btih:abababab'),
      throwsA(isA<TorrentError>()), // TODO:
    );
  });
  ...
  test('parse_long_hash', () {
    expect(
      () => AddParam.parse('magnet:?xt=urn:btih:ababababababababababab'),
      throwsA(isA<TorrentError>()), // TODO:
    );
  });
  ...
  test('parse_space_hash', () {
    expect(
      () => AddParam.parse('magnet:?xt=urn:btih: abababababababababab'),
      throwsA(isA<TorrentError>()), // TODO:
    );
  });
  ...
  test('parse_v2_hash', () {
    ...
    expect(p.infoHashes.hasV1, isFalse);
    expect(p.infoHashes.hasV2, isTrue);
    expect(
        p.infoHashes.v2,
        Digest(hex.decode(
  ...
  test('parse_hybrid_uri', () {
    ...
    expect(p.infoHashes.hasV1, isTrue);
    expect(
        p.infoHashes.v1,
        Digest(hex.decode(
          'cdcdcdcdcdcdcdcdcdcdcdcdcdcdcdcdcdcdcdcd',
